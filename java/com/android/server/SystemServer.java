...
public final class SystemServer {
    ...
    private void startOtherServices() {
        ...
        if (mFactoryTestMode != FactoryTest.FACTORY_TEST_LOW_LEVEL) {
            ...
            if(!disableNonCoreServices) {
    	       try {
                   Slog.i(TAG, "Condeco RGBLED Service");
                   ServiceManager.addService(Context.CONRGBLEDSYS_SERVICE, new ConRGBLEDService(context));
               } catch (Throwable e) {
                   reportWtf("Failure starting ConRGBLEDService Service", e);
               }
            }
	    ...
	    if(!disableNonCoreServices) {
    	       try {
                   Slog.i(TAG, "Kelvin Serial Service");
                   ServiceManager.addService("serialsys", new SerialSysService(context));
               } catch (Throwable e) {
                   reportWtf("Failure starting SerialSysService Service", e);
               }
            }

            if(!disableNonCoreServices) {
    	       try {
                   Slog.i(TAG, "Condeco BLE Service");
                   ServiceManager.addService(Context.CONBLESYS_SERVICE, new ConBLEService(context));
               } catch (Throwable e) {
                   reportWtf("Failure starting ConBLEService Service", e);
               }
            }
            ...
            if (!disableNetwork && !disableNetworkTime) {
                ...
                try {
                    mSystemServiceManager.startService(NetworkTimeUpdateService.Lifecycle.class);
                } catch (Throwable e) {
